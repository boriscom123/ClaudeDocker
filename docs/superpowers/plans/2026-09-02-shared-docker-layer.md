...
> **Исправление порядка, найденное при исполнении.** Изначально задача 4 гасила
> только `nginx`, `adminer` и `portainer`, оставляя `postgres` проекта `game`
> работать. Но общий `postgres` подключает тот же самый том — два постгреса на
> одном каталоге данных недопустимы (второй не стартует из-за `postmaster.pid`,
> а при неудачном стечении можно испортить данные). Поэтому том должен освободиться
> до подъёма общего слоя, а значит переключение выполняется одним окном вместе
> с задачами 5 и 6: сначала пишутся оверлеи и правится compose проекта `game`
> (без применения), затем гасится `game` целиком, поднимается общий слой и следом
> проекты. То же касается `uzbek_queue`: он ходит в тот же postgres.

Здесь начинается простой. Путь отката — `git checkout` в репозиториях проектов
плюс `docker compose up -d` в прежнем виде.
